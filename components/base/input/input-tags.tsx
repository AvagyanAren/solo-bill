...
const createTagEntries = (labels: string[]): TagEntry[] => {
    const occurrences = new Map<string, number>();

    return labels.map((label) => {
        const occurrence = occurrences.get(label) ?? 0;
        occurrences.set(label, occurrence + 1);
        return { id: `${encodeURIComponent(label)}-${occurrence}`, label };
    });
};
...
export const InputTags = ({
    ...
    const [internalEntries, setInternalEntries] = useState<TagEntry[]>(() => createTagEntries(defaultValue ?? []));
    const entries = isControlled ? createTagEntries(value ?? []) : internalEntries;
    ...
    const addTag = useCallback(
        (text: string) => {
            ...
            const newEntries = createTagEntries([...tags, trimmed]);
        ...
        [tags, isControlled, allowDuplicates, maxTags, validate, onChange, onTagAdded],
    );
